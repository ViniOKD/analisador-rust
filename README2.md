...
# Interpretador de bytecode para saídas Rust

## Como executar

O interpretador recebe como entrada um arquivo contendo bytecode.
O bytecode pode ser gerado pelo analisador.

Exemplo de execução:

```bash
python interpretador.py testeX_bytecode.txt
```

## Funcionalidades implementadas

- O interpretador detecta alguns erros em tempo de execução.
- As instruções de bytecode de saída geram resultados na saída padrão.
